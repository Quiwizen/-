...
function gethtml(fileurl,name,extension){
    ...
    setTimeout(()=>{
        for (let i = 0; i < 99; i++) {
            fs.access(fileurl+'\\'+ i +'.'+extension,(err)=>{
                if(err){
                    return false;
                }
                getbase64tag(fileurl+'\\'+ i +'.'+extension,name);
...
const extension = 'png'  //图片后缀名字
const fileurl = 'C:\\Users\\Quiwizen\\Desktop\\test'    //文件夹路径
const filename = 'name'  //输出html的命名
gethtml(fileurl,filename,extension)
// for (let i = 0; i < 99; i++) {
//     gethtml(fileurl + '0 ('+i+')', i ,extension)   
// }